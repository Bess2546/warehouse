...
export const MOVEMENT_CONFIG = {
  ...
  OUT_COUNT_THRESHOLD: 5,       // ไม่เห็น 5 ครั้งติดต่อกัน
  OUT_TIMEOUT_SEC: 120,         // หรือไม่เห็นนาน 2 นาที
...
export interface TagScanBuffer {
  ...
  // Scan tracking
  SeenCount: number;            // จำนวนครั้งที่เห็น (reset เมื่อไม่เห็น)
  MissedCount: number;          // จำนวนครั้งที่ไม่เห็น (reset เมื่อเห็น)
  TotalScans: number;           // จำนวน scan ทั้งหมดตั้งแต่เริ่ม track
  
  // RSSI tracking
  LastRssi: number;
  RssiHistory: number[];        // เก็บ RSSI ล่าสุด N ครั้ง
  AvgRssi: number;
  
  // Timestamps
  FirstSeenTime: Date;          // เห็นครั้งแรกเมื่อไหร่
  LastSeenTime: Date;           // เห็นล่าสุดเมื่อไหร่
  LastScanTime: Date;           // scan ล่าสุด (เห็นหรือไม่เห็นก็ตาม)
